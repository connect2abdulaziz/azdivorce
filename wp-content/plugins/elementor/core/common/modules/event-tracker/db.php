...
class DB extends Base_Object {
	...
	private function add_indexes() {
		...
		$index_exists = $this->wpdb->get_var( "SHOW INDEX FROM `{$table_name}` WHERE Key_name = 'created_at_index'" );
		if ( ! $index_exists ) {
			// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
	...
	public function __construct() {
		...
		// Check if table exists (case-insensitive for cross-platform compatibility). If not, create it.
		$query = $wpdb->prepare( 'SHOW TABLES LIKE %s', $wpdb->esc_like( $this->get_table_name() ) );

		// phpcs:disable WordPress.DB.PreparedSQL.NotPrepared
		$found_table = $wpdb->get_var( $query );
		if ( null === $found_table || strtolower( (string) $found_table ) !== strtolower( $this->get_table_name() ) ) {
